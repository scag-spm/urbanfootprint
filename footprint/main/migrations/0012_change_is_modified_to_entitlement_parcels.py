...
class Migration(SchemaMigration):
    ...
    def __init__(self):
        self.table_to_modify = 'entitlement_parcels_2016'
        self.column_name = 'is_modified'
        self.column_type = 'VARCHAR(10)' # This string will be passed directly to postgres

    def forwards(self, orm):
        for db_entity in DbEntity.objects.filter(table__contains=self.table_to_modify).values('schema', 'table'):
            # Ignore null fields returned by Django query
            if db_entity['schema']:
                # 1. Drop "new" column to cover cases where fix was manually deployed
                # 2. Add new column to tables
                sql  = '''
                    ALTER TABLE {schema}.{table} DROP COLUMN IF EXISTS {column_name};
                    ALTER TABLE {schema}.{table} ADD COLUMN {column_name} {column_type} DEFAULT 'false';
                    '''.format(
    ...
    models = {
        ...
        'main.scagdmlanduse': {
            'Meta': {'object_name': 'ScagDmLandUse'},
            u'builtform_ptr': ('django.db.models.fields.related.OneToOneField', [], {'to': "orm['main.BuiltForm']", 'unique': 'True', 'primary_key': 'True'}),
            'land_use_definition': ('django.db.models.fields.related.ForeignKey', [], {'to': "orm['main.ScagDmLandUseDefinition']"})
        },
        'main.scagdmlandusedefinition': {
            'Meta': {'object_name': 'ScagDmLandUseDefinition'},
            u'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
